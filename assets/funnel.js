...
var MIN_TIME_ON_PAGE_MS = 3000; // guarantee ads get at least this long to load before leaving

var SKELETON_MS = 900; // quick, snappy pause — feels like the popup is "processing"
...
function goToNextPage(url) {
  ...
  var remaining = Math.max(0, MIN_TIME_ON_PAGE_MS - elapsed);
  var waitTime = Math.max(SKELETON_MS, remaining); // only slows down if they tapped very fast
